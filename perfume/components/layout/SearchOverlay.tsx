  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#080808]/95 backdrop-blur-md flex flex-col">
      <div className="flex items-center justify-between px-6 py-5 border-b border-[#C9A84C]/20">
        ...
        <button onClick={() => setSearchOpen(false)} className="text-cream/60 hover:text-[#C9A84C] transition-colors">
          <X size={22} />
        </button>
      </div>
      ...
      <div className="max-w-2xl mx-auto w-full px-6 pt-10 pb-6">
        <div className="relative">
          <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#C9A84C]/60" />
          <input
            ref={inputRef}
            value={query}
            onChange={e => setQuery(e.target.value)}
            placeholder="Search for pieces, collections..."
            className="w-full bg-transparent border-b-2 border-[#C9A84C]/30 focus:border-[#C9A84C] outline-none pl-10 pr-4 py-3 text-xl font-serif text-cream placeholder-cream/20 transition-colors"
          />
        </div>
            ...
            {results.map(p => (
              <Link key={p.id} href={`/product/${p.slug}`}
                ...
                <div className="flex-1">
                  <p className="text-cream text-sm font-body">{p.name}</p>
                  <p className="text-cream/40 text-xs">{p.category} · ${p.price}</p>
                </div>
                <ArrowRight size={14} className="text-[#C9A84C]/40 group-hover:text-[#C9A84C] transition-colors" />
              </Link>
            ))}
            <Link href={`/search?q=${query}`} onClick={() => setSearchOpen(false)}
              className="block text-center text-[#C9A84C] text-xs tracking-[0.2em] uppercase py-3 font-body hover:underline">
              View All Results →
            </Link>
          </div>
        )}
        ...
        {query.length > 1 && results.length === 0 && (
          <p className="text-cream/40 text-sm mt-8 text-center font-body">No results for "{query}"</p>
        )}

        {!query && (
          <div className="mt-10">
            <p className="text-xs tracking-[0.2em] uppercase text-cream/30 font-body mb-4">Popular Searches</p>
