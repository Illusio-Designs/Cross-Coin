    ...
    <div className="pt-24 min-h-screen">
      <div className="max-w-3xl mx-auto px-6 py-10">
        <div className="text-center mb-14">
          <p className="text-[#C9A84C]/60 text-xs tracking-[0.3em] uppercase font-body mb-3">Help Center</p>
          <h1 className="font-serif text-5xl text-cream">FAQ</h1>
          <div className="gold-divider w-24 mx-auto mt-4" />
          <p className="text-cream/40 text-sm font-body mt-4">
            Can't find your answer? <a href="/contact" className="text-[#C9A84C] hover:underline">Contact us</a>
          </p>
        </div>
        ...
        <div className="space-y-10">
          {faqs.map(section => (
            <div key={section.category}>
              <h2 className="font-serif text-xl text-cream mb-4 pb-3 border-b border-[#C9A84C]/15">
                {section.category}
              </h2>
              <div className="space-y-2">
                {section.items.map(item => {
                  ...
                  return (
                    <div key={item.q} className="border border-[#C9A84C]/10 rounded-sm overflow-hidden">
                      <button
                        onClick={() => setOpenItem(isOpen ? null : key)}
                        className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-[#C9A84C]/5 transition-colors"
                      >
                        <span className="text-cream/80 font-body text-sm pr-4">{item.q}</span>
                        {isOpen
                          ? <Minus size={14} className="text-[#C9A84C] flex-shrink-0" />
                          : <Plus size={14} className="text-cream/40 flex-shrink-0" />
                        }
                      </button>
                      {isOpen && (
                        <div className="px-5 pb-4 bg-[#1a1a1a]/50">
                          <p className="text-cream/50 font-body text-sm leading-relaxed">{item.a}</p>
                        </div>
                      )}
                    </div>
                  );
