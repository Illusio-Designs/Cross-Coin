      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
              ...
              <button key={cat} onClick={() => setActiveCategory(cat)}
                className={`px-4 py-2 text-xs tracking-[0.15em] uppercase font-body rounded-full border transition-all ${
                  activeCategory === cat
                    ...
                    : 'border-[#C9A84C]/20 text-cream/60 hover:border-[#C9A84C]/50 hover:text-[#C9A84C]'
                }`}>
                {cat}
              </button>
            ))}
          ...
          <div className="flex items-center gap-3">
            <span className="text-cream/40 text-xs font-body">{filtered.length} items</span>
            {/* Sort */}
            <div className="relative">
              ...
              <ChevronDown size={12} className="absolute right-2 top-1/2 -translate-y-1/2 text-[#C9A84C]/60 pointer-events-none" />
            </div>
            {/* Filter toggle */}
        ...
        <div className="flex gap-8">
          ...
          {filtersOpen && (
            <aside className="w-56 flex-shrink-0">
              <div className="bg-[#1a1a1a] border border-[#C9A84C]/15 rounded-sm p-5 space-y-6 sticky top-28">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs tracking-[0.2em] uppercase text-cream font-body">Filters</h3>
                  <button onClick={() => setFiltersOpen(false)}><X size={14} className="text-cream/40" /></button>
                </div>

                {/* Price range */}
                <div>
                  <p className="text-xs tracking-[0.15em] uppercase text-cream/50 font-body mb-3">Price Range</p>
                  <div className="flex items-center gap-2 text-cream/60 text-xs font-body mb-2">
                ...
                <button onClick={() => { setActiveCategory('All'); setPriceRange([0, 500]); }}
                  className="text-[#C9A84C]/60 text-xs uppercase tracking-wider font-body hover:text-[#C9A84C] transition-colors">
                  Clear All
                </button>
              </div>
            </aside>
          ...
          <div className="flex-1">
            {filtered.length > 0 ? (
              ...
              <div className="text-center py-20">
                <p className="font-serif text-xl text-cream/40">No products found</p>
                <button onClick={() => setActiveCategory('All')} className="text-[#C9A84C] text-xs tracking-wider uppercase font-body mt-4 hover:underline">
                  Clear Filters
                </button>
              </div>
            )}
