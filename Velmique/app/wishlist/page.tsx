  ...
  return (
    <div className="pt-24 min-h-screen">
      <div className="max-w-7xl mx-auto px-6 py-10">
        <div className="mb-10">
          <p className="text-[#C9A84C]/60 text-xs tracking-[0.3em] uppercase font-body mb-2">Saved Items</p>
          <h1 className="font-serif text-4xl text-cream">Your Wishlist</h1>
          <div className="gold-divider w-16 mt-4" />
        </div>

        {wishlist.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-5">
            <Heart size={52} className="text-cream/10" />
            <p className="font-serif text-2xl text-cream/40">Your wishlist is empty</p>
            <p className="text-cream/30 text-sm font-body">Save pieces you love to revisit them later.</p>
              ...
              <div key={item.id} className="group relative">
                ...
                <div className="pt-3">
                  <Link href={`/product/${item.slug}`}>
                    <h3 className="font-serif text-sm text-cream hover:text-[#C9A84C] transition-colors">{item.name}</h3>
                  </Link>
                  <p className="text-[#C9A84C] text-sm font-body mt-1">${item.price}</p>
                </div>
              </div>
            ))}
          </div>
