...
export default function CartDrawer() {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className={`fixed right-0 top-0 h-full w-full max-w-md z-[90] bg-[#111] border-l border-[#C9A84C]/20 transition-transform duration-400 ease-out flex flex-col ${cartOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-[#C9A84C]/15">
          <div className="flex items-center gap-3">
            ...
            <span className="font-serif text-lg text-cream">Your Bag</span>
            {cartCount > 0 && <span className="bg-[#C9A84C]/20 text-[#C9A84C] text-xs px-2 py-0.5 rounded-full font-body">{cartCount}</span>}
          </div>
          <button onClick={() => setCartOpen(false)} className="text-cream/50 hover:text-[#C9A84C] transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center gap-4">
              ...
              <p className="font-serif text-xl text-cream/40">Your bag is empty</p>
              <p className="text-cream/30 text-sm font-body">Discover our luxury collections</p>
              ...
              {cart.map((item, i) => (
                <div key={`${item.id}-${item.size}-${i}`} className="flex gap-4 pb-5 border-b border-[#C9A84C]/10 last:border-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <Link href={`/product/${item.slug}`} onClick={() => setCartOpen(false)}>
                      <h4 className="font-serif text-sm text-cream hover:text-[#C9A84C] transition-colors truncate">{item.name}</h4>
                    </Link>
                    {item.size && <p className="text-cream/40 text-xs mt-0.5 font-body">Size: {item.size}</p>}
                    ...
                    <div className="flex items-center justify-between mt-3">
                      <div className="flex items-center gap-2 border border-[#C9A84C]/20 rounded-sm">
                        <button onClick={() => updateQuantity(item.id, item.quantity - 1, item.size)}
                          className="px-2 py-1 text-cream/60 hover:text-[#C9A84C] transition-colors">
                          <Minus size={12} />
                        </button>
                        <span className="text-cream text-sm w-4 text-center font-body">{item.quantity}</span>
                        <button onClick={() => updateQuantity(item.id, item.quantity + 1, item.size)}
                          className="px-2 py-1 text-cream/60 hover:text-[#C9A84C] transition-colors">
                          <Plus size={12} />
                        </button>
                      </div>
                      <span className="text-[#C9A84C] font-body text-sm">${(item.price * item.quantity).toFixed(2)}</span>
                    </div>
                  </div>
                  <button onClick={() => removeFromCart(item.id, item.size)}
                    className="text-cream/20 hover:text-red-400 transition-colors self-start mt-1">
                    <Trash2 size={14} />
                  </button>
                </div>
              ))}
        ...
        {cart.length > 0 && (
          <div className="border-t border-[#C9A84C]/15 px-6 py-5 space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-cream/50 text-sm font-body">
                <span>Subtotal</span>
                <span>${cartTotal.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-cream/50 text-sm font-body">
                <span>Shipping</span>
                <span className="text-[#C9A84C]">{cartTotal >= 150 ? 'Free' : '$15.00'}</span>
              </div>
              <div className="gold-divider" />
              <div className="flex justify-between text-cream font-serif text-lg">
                <span>Total</span>
            ...
            <Link href="/cart" onClick={() => setCartOpen(false)}
              className="block text-center text-cream/40 text-xs tracking-wider uppercase font-body hover:text-[#C9A84C] transition-colors">
              View Full Cart
            </Link>
          </div>
        )}
