...
/**
 * Whole-app error boundary for Velmique.
 *
 * Catches render errors below it so a single bad component never
 * blanks the entire site. Buffers errors onto window.__velmiqueErrors
 * for any monitor (Sentry / Datadog / custom) to drain. Dispatches a
 * 'velmique:error' CustomEvent for real-time pickup.
 */
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    ...
    if (typeof window !== 'undefined') {
      window.__velmiqueErrors = window.__velmiqueErrors || [];
      window.__velmiqueErrors.push({
        message: error?.toString(),
        stack: errorInfo?.componentStack,
        href: window.location.href,
        at: new Date().toISOString(),
      });
      if (window.__velmiqueErrors.length > 20) window.__velmiqueErrors.shift();
      try { window.dispatchEvent(new CustomEvent('velmique:error', { detail: { error, errorInfo } })); } catch { /* ignore */ }
    }
...
const styles = {
  container: { display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', backgroundColor: '#faf6ee', padding: '20px' },
