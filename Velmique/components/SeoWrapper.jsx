...
const FRONTEND_URL = process.env.NEXT_PUBLIC_FRONTEND_URL || 'https://velmique.com';
const API_URL      = process.env.NEXT_PUBLIC_API_URL      || 'https://api.crosscoin.in';

const TAG_PREFIX = 'velmique-seo-';
...
/* Upsert a managed meta/link element so swapping pages doesn't leave
   stale tags behind. Each tag gets a deterministic id like
   `velmique-seo-meta-description` so subsequent renders replace it. */
...
export default function SeoWrapper({ pageName, seoData, children }) {
  ...
  const defaults = useMemo(() => ({
    meta_title: 'Velmique',
    meta_description: 'Velmique — extrait, eau de parfum and discovery kits, composed in Grasse.',
    meta_keywords: 'velmique, perfume, fragrance, extrait, eau de parfum',
    canonical_url: null,
    meta_image: null,
  }), []);
