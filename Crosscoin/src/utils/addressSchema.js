 ...
 * storefront. Mirrors the Zod schemas in
 * Backend/routes/shippingAddressRoutes.js so a payload that passes
 * here will pass there.
 *
 * Used by:
 *   - CartDrawer's address form (validates before submit)
 *   - Profile address modal
 *   - Any future address form (react-hook-form + zodResolver)
 *
 * Quick usage with react-hook-form:
 *
 ...
 *   });
 *
 * Without react-hook-form (current CartDrawer pattern):
 *
 *   const result = addressSchema.safeParse(addressForm);
 *   if (!result.success) {
 *     const firstIssue = result.error.issues[0];
 *     showValidationErrorToast(firstIssue.message);
 *     return;
 *   }
 */
...
const realAddressGuard = (val) => {
  ...
  // Reject obvious placeholders that historically generated RTO.
...
/**
 * Convenience wrapper for the legacy callers that expect
 * { valid, errors, warnings } shape (CartDrawer.validateShippingAddress).
 * Returns the FIRST error message because that matches the existing toast UX.
 */
export function validateAddress(addr) {
  // Mirror legacy alias names the CartDrawer code uses.
  const candidate = {
