...
export default function MagicCheckoutTest() {
  ...
  // Delivery serviceability check (backend → iThink). Confirm the PIN delivers
  // before running the Magic checkout.
  ...
  const checkPincode = async () => {
    if (!/^\d{6}$/.test(pincode)) { setServ({ error: 'Enter a valid 6-digit PIN code.' }); return; }
    setCheckingPin(true); setServ(null);
    try {
      const r = await fetch(`${API}/api/serviceability/${pincode}?debug=1&cb=${Date.now()}`, {
        headers: { 'X-Brand-Name': BRAND, 'Cache-Control': 'no-cache' },
      });
      const d = await r.json();
      setServ(d);
      push(`GET /api/serviceability/${pincode}?debug=1 → response (iThink raw is in _debug)`, d);
    } catch (e) {
      setServ({ error: 'Check failed: ' + (e?.message || e) });
    } finally {
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 820, margin: '0 auto', padding: 24, fontFamily: 'DM Sans, system-ui, sans-serif', color: '#1a1a1a' }}>
        ...
        {/* ── Delivery serviceability (backend → iThink) ── */}
        <div style={box}>
          <h3 style={{ marginTop: 0, fontSize: 15 }}>Confirm delivery (serviceability)</h3>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <button
              ...
              style={{ background: '#111', color: '#fff', border: 0, borderRadius: 6, padding: '9px 16px', fontSize: 13, fontWeight: 600, cursor: checkingPin ? 'default' : 'pointer', opacity: checkingPin ? 0.6 : 1 }}
            >
          ...
          {serv && (
            <div style={{ marginTop: 12, fontSize: 14 }}>
              {serv.error ? (
                <span style={{ color: '#c62828' }}>{serv.error}</span>
              ) : serv.serviceable ? (
                <span style={{ color: '#2e7d32', fontWeight: 600 }}>
                  ✓ Deliverable{serv.cod_allowed ? ' · COD available' : ' · Prepaid only'}
                  {serv.estimated_delivery_days ? ` · ~${serv.estimated_delivery_days} days` : ''}
                </span>
              ) : (
                <span style={{ color: '#c62828', fontWeight: 600 }}>✗ Not deliverable to this PIN</span>
              )}
              {serv._debug && (
                <details style={{ marginTop: 8 }}>
                  <summary style={{ cursor: 'pointer', color: '#777', fontSize: 12 }}>iThink raw response + resolved host</summary>
                  <pre style={{ margin: '6px 0 0', fontSize: 11.5, whiteSpace: 'pre-wrap', wordBreak: 'break-word', background: '#f7f7f9', padding: 10, borderRadius: 6 }}>
                    {JSON.stringify(serv._debug, null, 2)}
                  </pre>
                </details>
              )}
            </div>
          )}
          <p style={{ color: '#999', fontSize: 12, marginBottom: 0, marginTop: 10 }}>
            Confirm the PIN delivers here before running Magic checkout. Uses the same backend serviceability the storefront uses.
          </p>
        </div>
