...
export default function WhatsAppChat() {
  ...
  const showGreet = greetOpen && !scrolled;

  return createPortal(
    <div className="fixed bottom-6 right-6 z-[1050] flex flex-col items-end gap-3">
      {showGreet && (
        <div className="flex items-center gap-2 rounded-2xl rounded-br-sm border border-line bg-paper px-3.5 py-2.5 shadow-card">
          <a
            href={waUrl}
            target="_blank"
          ...
          <button
            ...
            aria-label="Dismiss"
            className="text-lg leading-none text-ink-muted transition-colors hover:text-ink"
          >
            ×
          </button>
        </div>
      ...
      <a
        ...
        className="flex h-14 w-14 items-center justify-center rounded-full shadow-card transition-transform hover:scale-105"
