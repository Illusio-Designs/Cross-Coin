...
async function seedDefaultTemplates(brandId = 1) {
  // Fetch brand name/URL from settings for personalised footer
  const [storeName, storeUrl, supportPhone] = await Promise.all([
    settingsHelper.getSetting(brandId, 'STORE_NAME', 'Cross Coin'),
    settingsHelper.getSetting(brandId, 'STORE_URL', 'crosscoin.in'),
    settingsHelper.getSetting(brandId, 'SUPPORT_PHONE', ''),
  ]);
