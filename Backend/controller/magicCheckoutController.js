...
exports.createMagicOrder = async (req, res) => {
  try {
    ...
    return res.json({
      ...
      // When Magic is off, the exact reason Razorpay gave for rejecting 1CC
      // (surfaced so the test page shows WHY instead of guessing).
      magic_disabled_reason: magicEnabled ? undefined : (order._magic_disabled_reason || 'Razorpay rejected one_click_checkout (account not provisioned for Magic/1CC).'),
    });
  } catch (e) {
