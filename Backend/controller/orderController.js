  ...
  module.exports.createOrder = async (req, res) => {
    ...
    try {
      ...
      if (shippingAddress.pincode) {
        try {
          const provider = await shippingProviderFactory.getShippingProvider(1); // always iThink
          const warehousePincode = await settingsHelper.getSetting(1, 'ITHINK_WAREHOUSE_PINCODE',
            await settingsHelper.getSetting(1, 'DEFAULT_WAREHOUSE_PINCODE', '363641'));
  ...
  module.exports.trackOrderByAWB = async (req, res) => {
    try {
      // The route schema validates the query param as `awb`, but this handler
      // historically read `awb_number` — so tracking by AWB always 400'd
      // ("AWB number is required"). Accept both names.
      const awb_number = req.query.awb || req.query.awb_number;
      ...
      // Find order by AWB — the waybill can be stored in any of these columns
      // depending on the provider/booking path, so match all of them.
      const order = await Order.findOne({
        where: {
          [Op.or]: [
            { tracking_number: awb_number },
            { fship_waybill: awb_number },
            { fship_tracking_number: awb_number },
          ],
        },
        include: [
          {
  ...
  // ON DELETE CASCADE the associations declare.
  module.exports.adminDeleteOrder = async (req, res) => {
    ...
    try {
      ...
      // Remove child rows first (ignore ones whose table/column may not exist).
      const delChild = async (Model, where) => {
        try { await Model.destroy({ where, transaction: t }); } catch (e) { logger.warn(`[DeleteOrder] child cleanup skipped: ${e.message}`); }
      };
      await delChild(OrderItem, { order_id: id });
      await delChild(OrderStatusHistory, { order_id: id });
      await delChild(Payment, { order_id: id });
      await delChild(OrderShipment, { order_id: id });
      try {
        const { FShipLabelDownload } = require('../model/fshipLabelDownloadModel.js');
        if (FShipLabelDownload) await delChild(FShipLabelDownload, { order_id: id });
      } catch (_) { /* model optional */ }

      await order.destroy({ transaction: t });
      await t.commit();
      logger.info(`[DeleteOrder] Order ${orderNumber} (id=${id}) permanently deleted by user ${req.user?.id || 'unknown'}`);
      return res.json({ success: true, message: `Order ${orderNumber} deleted` });
    } catch (error) {
      if (t) await t.rollback().catch(() => {});
      logger.error(`[DeleteOrder] failed for order ${id}: ${error.message}`);
      return res.status(500).json({ success: false, message: `Failed to delete order: ${error.message}` });
    }
  };
