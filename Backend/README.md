...
8. Request validation middleware (Joi or Zod) so controllers stop hand-checking required fields.
9. Move cron from `node-cron` to Bull queues (persistence + dashboard + idempotency).
10. ~~`/api/metrics` endpoint with memory + queue stats.~~ **DONE**. Add slow-query log next.
