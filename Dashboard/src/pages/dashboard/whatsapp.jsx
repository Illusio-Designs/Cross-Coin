...
// Preview catalog — MUST mirror the backend's canonical templates
// (Backend/services/whatsappService.js → buildTemplates), which is what is
// actually seeded/sent. All lifecycle templates are brand-generic: {{1}}=customer
// name, {{2}}=brand (injected per message), then order-specific values. Do NOT
// hardcode a brand name here or the preview will misrepresent what customers get.
const TEMPLATES = {
  order_confirm:    { name:'order_confirmation_v2',     title:'Order Confirmed',    icon:'order_confirm',    category:'UTILITY',   body:'Thank you, {{1}}! 🎉\n\nYour *{{2}}* order *#{{3}}* (₹{{4}}) is confirmed and we\'re preparing it for dispatch now.\n\nWe\'ll share your tracking details as soon as it ships. We appreciate your trust in us!', footer:'Need help? Just reply to this message.', btn1:{type:'',text:'',val:''}, btn2:'' },
  order_shipped:    { name:'order_shipped_v2',          title:'Order Shipped',      icon:'order_shipped',    category:'UTILITY',   body:'Great news, {{1}}! 🚚\n\nYour *{{2}}* order *#{{3}}* has been dispatched and is heading your way.\n\n🔖 *Tracking (AWB):* {{4}}\n📍 *Track live:* {{5}}\n\nThanks for staying connected — we\'ll notify you the moment it\'s delivered.', footer:'Need help? Just reply to this message.', btn1:{type:'',text:'',val:''}, btn2:'' },
  out_for_delivery: { name:'order_out_for_delivery_v2', title:'Out for Delivery',   icon:'out_for_delivery', category:'UTILITY',   body:'Out for delivery today, {{1}}! 🚛\n\nYour *{{2}}* order *#{{3}}* is arriving today via {{4}}.\n\nPlease keep your phone handy and make sure someone is available to receive the package.', footer:'Need help? Just reply to this message.', btn1:{type:'',text:'',val:''}, btn2:'' },
  order_delivered:  { name:'order_delivered_v2',        title:'Order Delivered',    icon:'order_delivered',  category:'UTILITY',   body:'Delivered! ✅\n\nYour *{{2}}* order *#{{3}}* has arrived — we hope you love it, {{1}}! 😊\n\nThank you for shopping with *{{2}}*. If anything\'s not right, just reply here within 48 hours and we\'ll help right away.', footer:'Need help? Just reply to this message.', btn1:{type:'',text:'',val:''}, btn2:'' },
  cart_abandon:     { name:'cart_abandoned_v2',         title:'Cart Abandoned',     icon:'cart_abandon',     category:'MARKETING', body:'Hi {{1}}!\n\nIt looks like you left something behind — your *{{2}}* is still waiting in your *{{3}}* cart.\n\nComplete your purchase now with an extra *10% OFF* using code *{{4}}* — valid for the next 24 hours only!', footer:'Need help? Just reply to this message.', btn1:{type:'',text:'',val:''}, btn2:'' },
  cod_confirm:      { name:'cod_order_confirmation_v2', title:'COD Confirmation',   icon:'cod_confirm',      category:'UTILITY',   body:'Hi {{1}}! 🛒\n\nYour *{{2}}* order *#{{3}}* for ₹{{4}} (Cash on Delivery) has been placed successfully.\n\n📍 *Deliver to:* {{5}}\n\nPlease reply *YES* to confirm, or tap *Confirm Address* below. Reply *NO* if you\'d like to cancel.', footer:'Need help? Just reply to this message.', btn1:{type:'QUICK_REPLY',text:'✅ Confirm Address',val:''}, btn2:'✏️ Wrong Address' },
  review_request:   { name:'review_request_v2',         title:'Review Request',     icon:'review_request',   category:'MARKETING', body:'Thanks for your order, {{1}}! 🌟\n\nHow did we do? Please take a moment to rate your experience with *{{2}}* or leave a quick review — it helps other shoppers.\n\n👉 Share your feedback: {{3}}', footer:'Need help? Just reply to this message.', btn1:{type:'',text:'',val:''}, btn2:'' },
  return_initiated: { name:'order_cancelled_v2',        title:'Return / Cancelled', icon:'return_initiated', category:'UTILITY',   body:'Hi {{1}}, your *{{2}}* order *#{{3}}* has been cancelled.\n\n💳 *Refund:* {{4}}\n\nIf this was unexpected or you have any questions, just reply here and our team will help right away.', footer:'Need help? Just reply to this message.', btn1:{type:'',text:'',val:''}, btn2:'' },
  refund_update:    { name:'refund_processed_v2',       title:'Refund Processed',   icon:'refund_update',    category:'UTILITY',   body:'Hi {{1}}, your refund for the *{{2}}* order *#{{3}}* has been initiated.\n\n💰 *Amount:* ₹{{4}}\n🏦 *Mode:* {{5}}\n⏳ *Expected credit:* 5–7 working days\n\nIf you don\'t see it within 7 working days, reply here and we\'ll look into it.', footer:'Need help? Just reply to this message.', btn1:{type:'',text:'',val:''}, btn2:'' },
};
...
// Positional sample values for the preview — aligned to the canonical templates'
// leading order: {{1}}=name, {{2}}=brand, {{3}}=order #, {{4}}=amount, {{5}}=link.
const SAMPLES = ['Rushikesh','CrossCoin','CC-20240601-0042','1,299','https://crosscoin.in/track','SAVE10','Surat, Gujarat - 395006','BlueDart Express','CrossCoin Ankle Socks'];
