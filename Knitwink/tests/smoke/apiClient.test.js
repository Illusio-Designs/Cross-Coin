/**
 * Smoke tests for the hardened fetch wrapper.
 *
 * Tests the parts that matter for production safety: header injection,
 * CSRF mirror, timeout, error categorisation, dual-shape envelope.
 *
 * fetch + Cookies + toast are mocked so the test runs without network.
 */
...
describe('apiClient — headers', () => {
  test('attaches X-Brand-Name on every request', async () => {
    ...
    expect(headers['X-Brand-Name']).toBe('knitwink');
  });
...
describe('apiClient — error handling', () => {
  ...
  test('suppressErrorToast skips the toast', async () => {
    mockFetch(async () => jsonResponse({}, { status: 500 }));
    await expect(
      apiClient.get('/api/boom', { suppressErrorToast: true }),
    ).rejects.toBeDefined();
