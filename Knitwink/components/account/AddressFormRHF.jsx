...
/**
 * react-hook-form address form bound to the shared Zod schema.
 *
 * Drop into any new account / checkout flow:
 *
 *   <AddressFormRHF
 ...
 *
 * Built-in features:
 *   - Field-level Zod errors surface under each input.
 *   - On blur of pincode/phone, debounces a /api/orders/check-address-quality
 *     call. Shows the COD eligibility + recommendation inline so the user
 *     knows BEFORE submitting whether COD is available.
 *   - Landmark field included (improves delivery + bumps the quality score).
 *
 * The CartDrawer keeps its legacy form for now (live checkout, can't
 * tolerate behavior changes). Use this for new forms.
 */
...
export default function AddressFormRHF({
  ...
  // Address-quality probe — fires when the user finishes typing both
  // a 6-digit pincode and a 10-digit phone. Debounced to 600ms so we
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        if (!cancelled) {
          // Dual-shape — read either response.data.score or response.score
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      {/* Address-quality strip: shown only after both pincode + phone parse. */}
      {quality && (
        <div
          role="status"
          aria-live="polite"
