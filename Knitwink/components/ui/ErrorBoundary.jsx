...
/**
 * Whole-app error boundary for Knitwink.
 *
 * Mounted once near the root in app/layout.jsx. Catches every render-time
 * exception below it so a single bad component never blanks the whole
 * site. Note: this only catches RENDER errors. Async errors in effects /
 * event handlers go through utils-level error toasts via apiFetch().
 *
 * On error:
 *   - Buffers the error onto window.__knitwinkErrors (max 20) so a
 *     monitor can drain them without code changes.
 *   - Dispatches a 'knitwink:error' CustomEvent for real-time pickup.
...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    ...
    if (typeof window !== 'undefined') {
      window.__knitwinkErrors = window.__knitwinkErrors || [];
      window.__knitwinkErrors.push({
        message: error?.toString(),
        stack: errorInfo?.componentStack,
        href: window.location.href,
        at: new Date().toISOString(),
      });
      if (window.__knitwinkErrors.length > 20) window.__knitwinkErrors.shift();

      try {
        window.dispatchEvent(new CustomEvent('knitwink:error', { detail: { error, errorInfo } }));
      } catch { /* ignore */ }
    }
...
const styles = {
  ...
  buttonSecondary: { background: '#374151', color: '#fff' },
  buttonGhost: { background: '#fff', color: '#111', borderColor: '#e5e7eb' },
  support: {
    fontSize: '13px',
    color: '#6b7280',
    margin: '8px 0 0 0',
