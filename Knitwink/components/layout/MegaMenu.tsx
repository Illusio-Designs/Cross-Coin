...
export function MegaMenu({ activeMenu }: MegaMenuProps) {
  const [activeTab, setActiveTab] = useState<Record<MenuKey, string>>({ men: 'Shoes', women: 'Shoes', sale: '' })
  ...
  const panel = (menu.panels as Record<string, typeof menu.panels[keyof typeof menu.panels]>)[currentTab]
                ...
                <li key={link.label}>
                  <Link
                    ...
                    className={cn(
                      ...
                      (link as { bold?: boolean }).bold ? 'font-medium text-gray-800' : 'text-gray-600'
                    )}
                  >
                    {link.label}
                  </Link>
