          ...
          {serviceability && (
            <div className={`pdp-pin-result${serviceability.error ? '' : serviceability.serviceable ? ' ok' : ' fail'}`}>
              {serviceability.error ? (
                ...
                <><Icon name="Check" size={14} /> Delivery to <strong>{pincode}</strong>{serviceability.cod_available && <span className="pdp-cod">COD available</span>}</>
              ) : (
                <><Icon name="X" size={14} /> Not deliverable to <strong>{pincode}</strong></>
              )}
            </div>
          )}
        </div>
