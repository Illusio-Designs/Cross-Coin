...
/* Numbered pagination — windowed range, styled with the brand's own tokens.
   Shows: ‹  1 … n-1 [n] n+1 … N  › (matches the Crosscoin behaviour). */
...
export default function Pagination({ page = 1, totalPages = 1, onChange, disabled = false }) {
  ...
  const btn = (active) => ({
    ...
    borderRadius: 10,
    border: '1px solid ' + (active ? 'var(--ink, #111)' : 'var(--line, #e5e7eb)'),
    background: active ? 'var(--ink, #111)' : '#fff',
  ...
  return (
    <nav aria-label="Pagination" className="blog-pagination"
      ...
      {items.map((it, i) =>
        it === '…' ? (
          <span key={`e${i}`} style={{ color: 'var(--ink-soft, #9ca3af)', padding: '0 4px' }}>…</span>
        ) : (
